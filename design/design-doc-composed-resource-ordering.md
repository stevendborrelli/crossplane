...
This document proposes implementing resource dependency in that Crossplane
...
the Composite Resource is deleted all the Composed resources are marked for deletion
in parallel.
...
* Let Crossplane's reconciler sequence real create and delete calls: don't
  ...
  which resources are yet to be created.
...
* Default to the common case destroy in the reverse of create order with an option for the less common case where a replacement should be created before its predecessor is destroyed.
* Keep the mechanism consistent with how the rest of the function pipeline
  already works. Accumulate steps through the pipeline.
...
* Changing how a provider determines a managed resource's readiness. For
  ...
  already produces.
* Making the function pipeline run during XR deletion, which will be addressed in #7242.

## Proposal

Add `dependencies` as a top-level field to the function protocol and implement a directed acyclic graph in the core rendering engine based on the desired dependencies returned from the function pipeline. Update the Composed Resource
Applicator to apply and delete resources based on the dependency graph.
...
already carry full resource payloads on every call.

Dependencies are defined as resource pairs using desired composed resource names, so
that any step in the pipeline can create and modify the dependency graph:

```text
[ // (resource, depends_on)
  (subnet-a, vpc), (subnet-b, vpc), (route-table-a, vpc), (route-b, route-table-a)
]
```

### Depending on a Required Resource

Dependencies on resources outside the Composite resources can also be defined using Kubernetes
references and selectors to support use cases where creation depends on the existence of a
centralized Configuration or a resource like a shared VPC.
...
Because the XR does not own the lifecycle of a Required resource, the effect
a dependency is different than within the composition:

* **It only supports Creation ordering** `create_resource_before_destroying_dependency` is not a valid setting and should be rejected by the core engine.
* **Missing/Multiple requirements affect the graph** Crossplane returns an empty `Resources` message when a requirement matched no objects, which will block the creation of Composed resources. When a requirement matches several objects and `name` is unset, all of them must be ready to unblock creation.
* **Readiness is read from the object.** Ready means a `Ready: True` status condition,
  or, for object kinds that have no `Ready` condition at all, existence. We may need to examine the real-world effects of this, as `Ready` state is often set at the end of the pipeline.
...
`design/assets/design-doc-composed-resource-ordering/notes-implementation-kickoff-ordering.md`.
