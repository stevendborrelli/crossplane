...
func AsEdges(deps []*fnv1.Dependency) []ordering.Edge {
	...
	for _, d := range deps {
		e := ordering.Edge{
			...
			CreateBeforeDestroy: d.GetCreateResourceBeforeDestroyingDependency(),
		}
...
// dependencyKey uniquely identifies an edge.
func dependencyKey(d *fnv1.Dependency) string {
	t := d.GetComposedResource()
	if r := d.GetRequiredResource(); r != nil {
		t = "required/" + r.GetRequirementName() + "/" + r.GetNamespace() + "/" + r.GetName()
	}

	return d.GetResource() + "->" + t
}

// staleEdgeNames summarises pruned edges for an event message.
func staleEdgeNames(edges []ordering.Edge) string {
	...
	for _, e := range edges {
		...
		if t == "" {
			t = "requirement " + e.DependsOn.RequirementName()
		}
...
// blockedMessage summarises the resources ordering held back this reconcile.
//
